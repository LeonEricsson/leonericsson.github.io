...
export default function Blog({ blogPosts, reviews }) {
  ...
  <div className="relative pl-[25rem] py-12">
    ...
    {/* Main blog section */}
    <h2 className="text-4xl mt-8 mb-6 font-cardo italic text-deep-charcoal">
      Blog posts
    </h2>
    ...
    <ul className="list-none space-y-4">
      {blogPosts.map(({ id, title, date }) => (
        <li key={id} className="flex items-baseline border-b border-subtle-line pb-4">
          ...
          <Link href={`/blog/${id}`}>
            <span className="text-deep-charcoal font-cardo italic text-lg hover:text-sepia-accent cursor-pointer transition-colors duration-300">{title}</span>
          </Link>
        </li>
      ))}
    </ul>

    <h2 className="text-4xl mt-16 mb-6 font-cardo italic text-deep-charcoal">
      Literature reviews
    </h2>
    <div className="w-16 h-px bg-subtle-line mb-8"></div>
    <ul className="list-none space-y-4">
