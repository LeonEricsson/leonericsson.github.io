...
Again, experiments begin with IsoFLOPs curves, this time they reveal a polynomial relationship between granularity and training loss, with loss decreasing with increased granularity at first, then after reaching a minimum, begins to increase. Optimal granularity of around 12 is stable across a wide range of FLOPs budgets, far beyond what we're seeing today in our SOTA MoEs. They do however note that poor routing balance shifts the optimum towards coarser granularity (fewer large experts).

##### Shared Expert Ratio
is explored through a series of experiments where the number of active experts ($E_a + E_s$), the activation ratio, and the model size is kept fixed. Varying shared expert ratio is achieved by substituting $E_a$ for $E_s$, exploring configurations from fully specialized ($S = 0$%), to highly shared ($S = 83.3$%). The findings reveal that picking a optimal shared ratio depends on the compute budget. Generally however, minimum loss is achieved at a low ratio, while having no shared experts results in suboptimal performance. Lower budgets < 10^20 benefit from higher shared ratios ($S = 16.7$%) whereas larger budgets > 10^20 achieve greater effeciency at lower ratio ($S = 8.3$%). Since todays models typically fall in the > 10^20 range, the practical suggestion becomes to deploy a single shared expert.
